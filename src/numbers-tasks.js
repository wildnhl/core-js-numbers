...
function getAverage(value1, value2) {
  return value1 + value2 !== Infinity ? (value1 + value2) / 2 : value1;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const g = Math.sqrt(x1 ** 2 + y1 ** 2);
  const g1 = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos((x1 * x2 + y1 * y2) / (g * g1));
}
...
function getParallelepipedDiagonal(a, b, c) {
  const multiple = a ** 2 + b ** 2 + c ** 2;
  return Math.sqrt(multiple);
}
...
function roundToPowerOfTen(num, pow) {
  const spec = num / 10 ** pow;
  return pow === 0 ? num : Math.round(spec) * 10 ** pow;
}
